...
function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!email && !password) {
        await api.auth.login({ email: "guest@poly.taste", password: "guest_password" });
      } else {
      ...
      <div className="flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-[420px]">
          ...
          <div className="mt-6 flex flex-col gap-3">
            ...
            <Button 
              variant="ghost" 
              className="w-full h-12 text-muted-foreground hover:bg-accent"
              type="button"
              onClick={() => {
                // Use the backend login with our mock credentials to act as a guest
                api.auth.login({ email: "guest@poly.taste", password: "guest_password" })
                  .catch(() => {})
