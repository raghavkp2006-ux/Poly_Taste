...
# Add parent directory to path so we can import routers.spotify
...
def run_evaluation():
    db = SessionLocal()
    try:
        token = get_valid_access_token(db, TEST_USER_ID)
    ...
    # Fill in missing genres using cache logic if needed
    ...
    for i, track in enumerate(top_20):
        ...
        # For simplicity in this script, we just look at matched_genres overlap
        if len(set(track["matched_genres"]).intersection(set(removed_genres))) >= 2:
            found_related = True
